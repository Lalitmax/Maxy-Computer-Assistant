...
def open(self, output=True, match_closest=False):
    ...
    if val == (""):
        ...
        if "," in val:
          ...
          return str( features.open_things(val, output=output, match_closest=match_closest))
            # return 1
...
def close(self, output=True, match_closest=False):
    ...
    if "," in val:
       ...
       return features.close_things(val, output=output, match_closest=match_closest)
