...
import math  # calculate some math problem
import time

import wikipedia  # for search information
...
import screen_brightness_control as pct # for set brightness
import sys  # no
...
def doooo(text):
    ...
    if text in ["exit", "quit", "stop"]:
        ...
        speak(autput.text)
        printtext(autput.text)
    elif text in ['open music', 'play music', 'music open', 'music play','play song']:
    ...
    elif text in ["open vs code",'vs code open']:
        os.startfile('code')
        speak("opening vs code")
        printtext("opened vs code")
    elif text in["open github","github open","open github.com","github.com open"]:
        webbrowser.open('https://github.com')
        speak("Opening github")
        printtext("opened Github.com")
        ...
        if yesnone !=1:
            ...
            if text in ["open recycle bin","recycle bin"]:
                ...
                webstr2 = "https://www."+webstr1+".com"
                webbrowser.open(webstr2)
                print(webstr1)
                speak("opening "+webstr1)
                printtext("opened "+webstr1)
        ...
        printtext("I am not able to execute this task, but here are some results")
        speak("I am not able to execute this task, but here are some results")
        pwt.search(text)
...
#  for icon
    ...
    wind.minsize(width=1420, height=870)
    wind.geometry("1200x850")
    ...
    # side 1 space
    mylabel3 = Label(wind,font=("arial",20 ),bg="#108cff",justify="left",border=3,width=2   )
    mylabel3.pack(side='left')
    # side 2 space
    mylabe5 = Label(wind,font=("arial",20),bg='#108cff',justify="left" ,border=3,width=2)
    mylabe5.pack(side='right')
     # label 1
    mylabel2 = Label(wind,font=("arial",20 ),bg="#1570CB",justify="left",border=0  )
    mylabel2.pack(pady = 67)
